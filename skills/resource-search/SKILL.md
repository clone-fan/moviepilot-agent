...
- This skill owns torrent/resource discovery requests. Do not route those through generic CLI/API first.
- Resolve media identity once, then check library/subscription only when it affects the next action or prevents duplicate acquisition.
- Present compact choices; do not silently start downloads. Manual downloads require explicit user confirmation after candidate resources are shown.
- For TV/anime, season choice matters. If the user asks to subscribe, confirm the season unless it is explicitly provided.
- If search fails, check recognition quality, site scope, and site health before concluding no resource exists.
...
- TV subscriptions must name the season; omitting season means season 1 only, so multi-season requests require separate season handling.
- If no resources appear, inspect site scope/health and recognition quality once before saying unavailable.

## Distilled Resource Search Flow

This skill owns tracker/resource discovery. Keep the flow compact and site-aware,
but do not over-probe sites before every normal search.
...
4. Present a compact resource card with quality, size, seeders/site, and the
   safest next choices.
5. Require explicit confirmation before starting downloads unless the user gave
   a direct link and explicitly asked to download it.
