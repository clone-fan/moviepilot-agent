...
## 最小验证路径
状态类：查权威工具一次即可；配置类：查现状 → 最小修改 → 复查关键字段；Agent/Git：先从 `/config/agent/runtime/git/REPO_SSH_MAP.md` 或对应 git-maintenance skill 定位实际仓库路径，再在真实仓库执行 `git status` / fetch / audit；不要把 `/config/agent` 当成 Git 工作区；心跳：固定脚本或渲染断言，不由 AI 生成数据。
